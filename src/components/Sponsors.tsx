export default function Sponsors() {
  ...
  const getCategoryClasses = (category: string) => {
    const classes = {
      Platinum:
        "w-24 h-24 text-2xl bg-slate-800/70 backdrop-blur-sm border-4 border-gray-300/50",
    ...
    >
      ...
      <div className="absolute inset-0">
        ...
        <div
          ...
          style={{
            backgroundImage:
              "url('https://images.unsplash.com/photo-1534796636912-3b95b3ab5986?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=400&q=80')",
            animationDelay: "2s",
          }}
        ></div>
        ...
        <div
          className="absolute top-2/3 right-1/4 w-1 h-1 bg-cyan-400 rounded-full animate-twinkle opacity-80"
          style={{ animationDelay: "1.5s" }}
        ></div>
        <div
          className="absolute top-1/2 left-3/4 w-1.5 h-1.5 bg-purple-400 rounded-full animate-twinkle opacity-60"
              ...
              <div key={index} className="text-center group">
                <div
                  className={`${getCategoryClasses(
                    sponsor.category
                  )} rounded-2xl flex items-center justify-center font-bold text-gray-300 mx-auto mb-3 shadow-lg group-hover:shadow-xl group-hover:shadow-cyan-500/25 transition-all cursor-pointer overflow-hidden`}
                >
                  <div
